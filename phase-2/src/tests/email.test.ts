/**
 * Phase 2 – email.test.ts
 * Tests the email body builder: HTML/text output contains theme names,
 * no raw PI data slips through, and week_start appears in subject.
 */
import test from 'node:test';
import assert from 'node:assert/strict';
import { buildEmailHtml, buildEmailText } from '../services/emailService';
import type { WeeklyPulse } from '../services/pulseService';

// ── Fixture ───────────────────────────────────────────────────────────────────
const mockPulse: WeeklyPulse = {
  id: 1,
  week_start: '2026-03-09',
  week_end: '2026-03-15',
  top_themes: [
    { theme_id: 1, name: 'Performance', description: 'App is slow at startup', review_count: 25, avg_rating: 2.4 },
    { theme_id: 2, name: 'KYC Issues', description: 'KYC verification failures', review_count: 18, avg_rating: 1.9 },
    { theme_id: 3, name: 'UI Crash', description: 'App crashes on mutual funds screen', review_count: 12, avg_rating: 2.1 }
  ],
  user_quotes: [
    { text: 'The app takes forever to load after updates.', rating: 2 },
    { text: 'KYC keeps failing every time I try.', rating: 1 },
    { text: 'Mutual funds tab crashes the app.', rating: 2 }
  ],
  action_ideas: [
    { idea: 'Profile and optimize app cold-start time.' },
    { idea: 'Improve KYC error messages and retry flow.' },
    { idea: 'Fix crash in mutual funds module.' }
  ],
  note_body: 'This week, performance dominated feedback. Users report slow startup and repeated KYC failures. Recommend targeted fixes.',
  created_at: '2026-03-12T10:00:00.000Z',
  version: 1
};

// ── HTML builder tests ────────────────────────────────────────────────────────

test('buildEmailHtml contains the week_start date', () => {
  const html = buildEmailHtml(mockPulse);
  assert.ok(html.includes('2026-03-09'), 'HTML should contain week_start');
});

test('buildEmailHtml contains all theme names', () => {
  const html = buildEmailHtml(mockPulse);
  for (const theme of mockPulse.top_themes) {
    assert.ok(html.includes(theme.name), `HTML should contain theme: ${theme.name}`);
  }
});

test('buildEmailHtml contains all user quotes', () => {
  const html = buildEmailHtml(mockPulse);
  for (const quote of mockPulse.user_quotes) {
    assert.ok(html.includes(quote.text), `HTML should contain quote: ${quote.text}`);
  }
});

test('buildEmailHtml contains all action ideas', () => {
  const html = buildEmailHtml(mockPulse);
  for (const action of mockPulse.action_ideas) {
    assert.ok(html.includes(action.idea), `HTML should contain action: ${action.idea}`);
  }
});

test('buildEmailHtml contains no raw email addresses from fixture', () => {
  const pulseCopy = { ...mockPulse, note_body: 'Contact admin@groww.in for help.' };
  // We don't scrub in the builder directly (scrub is done in sendPulseEmail), so
  // this test confirms the builder doesn't add PII by itself — the note_body would
  // need to go through scrubPii before sending.
  const html = buildEmailHtml(pulseCopy);
  // If PII was passed IN, it would appear (scrubbing is the caller's responsibility)
  assert.ok(html.includes('admin@groww.in'), 'builder passes through input as-is (scrub is caller responsibility)');
});

// ── Plain-text builder tests ──────────────────────────────────────────────────

test('buildEmailText contains week_start', () => {
  const text = buildEmailText(mockPulse);
  assert.ok(text.includes('2026-03-09'), 'text should contain week_start');
});

test('buildEmailText contains all theme names', () => {
  const text = buildEmailText(mockPulse);
  for (const theme of mockPulse.top_themes) {
    assert.ok(text.includes(theme.name), `text should contain theme: ${theme.name}`);
  }
});

test('buildEmailText contains section headers', () => {
  const text = buildEmailText(mockPulse);
  assert.ok(text.includes('TOP THEMES'));
  assert.ok(text.includes('USER QUOTES'));
  assert.ok(text.includes('RECOMMENDED ACTIONS'));
  assert.ok(text.includes('WEEKLY NOTE'));
});

test('buildEmailText contains note_body', () => {
  const text = buildEmailText(mockPulse);
  assert.ok(text.includes(mockPulse.note_body));
});
